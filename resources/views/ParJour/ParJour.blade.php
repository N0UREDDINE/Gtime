    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <div class="date-bar" style="width: 90%; text-align: center; margin-top:1px;">
                <span style="font-weight: bold; font-size: 18px;">Date actuelle: {{ $dt }}</span>
            </div>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if (session()->has('success'))
                <div id="flash-message" style="width: 400px; position: fixed; top: 150px; right: 10px"
                    class="bg-green-500 text-white py-2 px-4 rounded">{{ session()->get('success') }}</div>
            @endif
                ...
                <form enctype="multipart/form-data" method="get" action="{{ route('ParJour') }}" autocomplete="off">
                    ...
                    <div class="flex items-center">
                        ...
                        <input type="text" name="dt" value="{{ $dt }}"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-200 ml-2">
                        <button class="search rech bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow ml-2"
                            name="rechercher" id="search">Rechercher</button>
                    </div>
                </form>
            </div>
